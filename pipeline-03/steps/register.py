...
import threading
import time
from utils import start_mlflow_ui, upload_to_s3, download_from_s3, download_cacert
...
def build_sklearn_sagemaker_model(role, featurizer):
    ...
    print("[DEBUG] Inside build_sklearn_sagemaker_model.")
    ...
    class SklearnRequestTranslator(CustomPayloadTranslator):
        ...
        def deserialize_payload_from_stream(self, stream) -> pd.DataFrame:
            ...
            return df
    print("[DEBUG] Inside build_sklearn_sagemaker_model. created Request Translator")
    class SklearnModelSpec(InferenceSpec):
        ...
        def load(self, model_dir: str):
            ...
            return model
    print("[DEBUG] Inside build_sklearn_sagemaker_model. created SklearnModelSpec")
    ...
    print("[DEBUG] Inside build_sklearn_sagemaker_model. created SchemaBuilder")
    model_file_path="sklearn_model/sklearn_model.joblib"
    os.makedirs(os.path.dirname(model_file_path), exist_ok=True)
    joblib.dump(featurizer, model_file_path)
    print("[DEBUG] Inside build_sklearn_sagemaker_model. Building model")
...
def register(role, featurizer_model, booster, 
             ...
             experiment_name="main_experiment", run_id="run-01"):
    import subprocess
    # print("[DEBUG] installing ca certificates")
    # subprocess.run("apt-get update && apt-get install -y ca-certificates && update-ca-certificates", shell=True, check=True)
    os.environ["REQUESTS_CA_BUNDLE"]="/etc/ssl/certs/ca-certificates.crt"
    os.environ["AWS_CA_BUNDLE"]="/etc/ssl/certs/ca-certificates.crt"
    print("[DEBUG] Downloading certificates.")
    cert_path = "./certs/cacert.pem"
    # download_cacert(cert_path)
    ...
    print("[DEBUG] Starting MLflow UI for model register")
    threading.Thread(target=start_mlflow_ui, daemon=True).start()
    time.sleep(60)
    print("[DEBUG] Starting model registering...")   
    mlflow.set_tracking_uri(os.environ["MLFLOW_TRACKING_ARN"])
    mlflow.set_experiment(experiment_name=experiment_name)
    with mlflow.start_run(run_id=run_id) as run:
        with mlflow.start_run(run_name="Register", nested=True):
            mlflow.autolog()
            print("[DEBUG] Starting sklearn model building...") 
            sklearn_model = build_sklearn_sagemaker_model(role=role, featurizer=featurizer_model)
            print("[DEBUG] Success xgboost model building...") 
            xgboost_model = build_xgboost_sagemaker_model(role=role, booster=booster)
            print("[DEBUG] Success model building...") 
            ...
            # print("[DEBUG] Uploading model evaluation report...")
            # eval_report_str = json.dumps(model_report_dict)
            # s3_client = boto3.client("s3")
            # s3_client.put_object(
            #     Bucket=bucket_name,
            #     Key=f"{model_package_group_name}/evaluation-report/{eval_file_name}.json",
            #     Body=eval_report_str,
            #     ContentType="application/json"
            # )
            eval_report_s3_uri = f"s3://{bucket_name}/{model_package_group_name}/evaluation-report/{eval_file_name}.json"
            print("[DEBUG] Uploaded")
            # s3_fs = s3fs.S3FileSystem(session=boto_session)
